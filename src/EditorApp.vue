<template>
  <main class="editor-shell">
    <header class="editor-tools">
      ...
      <button class="tool-button" @click="command('formatBlock', 'h1')">H1</button>
      <button class="tool-button" @click="command('formatBlock', 'h2')">H2</button>
      <button v-for="color in colors" :key="color" class="color-button" :style="{ background: color }" :aria-label="`Text color ${color}`" @click="command('foreColor', color)" />
      <input v-model="customColor" class="color-picker" type="color" aria-label="Choose text color" @input="command('foreColor', customColor)" />
    ...
    <article ref="editor" class="page" contenteditable="true" spellcheck="true" data-placeholder="Start writing…" @input="save" />
  </main>
</template>
