<template>
  <main class="thank-shell">
    ...
    <template v-if="activeView === 'notes'">
      ...
      <EditorApp :key="selectedDate" :storage-key="entryKey" :placeholder="prompt" @saved="rememberDate" />
    </template>
      ...
      <section v-if="activeView !== 'notes'" :key="activeView" ref="visual" class="archive" :class="`archive-${activeView}`">
        ...
        <template v-if="entries.length">
          ...
          <div v-else-if="activeView === 'sky'" class="sky">
            ...
            <button v-for="(date, index) in entries" :key="date" v-motion class="star" :style="starStyle(index)" :initial="{ opacity: 0, scale: 0 }" :enter="{ opacity: 1, scale: 1, transition: { delay: index * 65, type: 'spring' } }" @click="openDate(date)"><span>★</span><small>{{ shortDate(date) }}</small></button>
          </div>
